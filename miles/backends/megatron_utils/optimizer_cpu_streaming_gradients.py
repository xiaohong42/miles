...
One deliberate difference: a parameter whose gradient is None in a step is skipped,
where stock HDO re-steps it with the previous step's cached host gradient.
"""
...
def add_optimizer_cpu_streaming_gradients_argument(parser):
    parser.add_argument(
        ...
        help=(
            ...
            "preserves FP32 master weights and optimizer-state dtypes. A parameter with no "
            "gradient in a step is skipped (stock HDO re-uses the previous step's cached gradient)."
        ),
    )
    return parser
