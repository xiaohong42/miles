...
def half_precision_output_kwargs(model_module) -> dict:
    ...
    model means a second copy of the whole ``[T, V]`` logit tensor -- 7.9 GiB at T=16384 and a
    129280-entry vocabulary. Everything downstream of the log-prob pass upcasts the slice it
    actually reads: ``calculate_log_probs_and_entropy`` copies each response chunk to fp32 and the
    fused vocab-parallel cross entropy starts with a cast. At long context those slices are a small
    fraction of ``T``. Megatron's own RL path selects the flag the same way, see
    ``megatron/rl/rl_utils.py``.

    Returns an empty dict when the model is not wrapped, since a bare ``GPTModel`` does not accept
    the argument.
    """
    module = model_module
