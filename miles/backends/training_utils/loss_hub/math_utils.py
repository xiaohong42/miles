...
def calculate_log_probs_and_entropy(
    ...
    if logits.size(0) != 0:
        ...
        # compute_log_probs and compute_entropy both upcast their input, so the branches above
        # return fp32 whatever the logits dtype is. The empty branch has to match: with bf16
        # logits it would otherwise return bf16, and under context parallelism a rank whose slice
        # holds no response token takes this branch while the others do not. They then meet in one
        # all-reduce over the concatenated response tensors, disagreeing on dtype. That is a hang,
        # not an error -- the collective never completes and the job dies to the NCCL watchdog.
