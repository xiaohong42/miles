...
def test_an_unknown_target_kind_gets_the_permissive_gemm_minimum():
    ...
    target = Target("vulkan -max_shared_memory_per_block=32768 -max_threads_per_block=1024 -thread_warp_size=32")
    assert target.kind.name not in tiling._MIN_GEMM_N_PER_WARP
    assert tiling.DeviceLimits.from_target(target).min_gemm_n_per_warp == tiling._MIN_GEMM_N_PER_WARP_DEFAULT
...
def test_an_inapplicable_block_h_is_recognised_as_a_tiling_rejection():
    """`heads % block_H != 0` means the candidate does not apply. Recognition only decides whether
    the rejection is logged as expected; the search itself retries on any rejection."""
    rejection = AssertionError("heads (48) should be a multiple of block_H (32)")
    assert tiling.is_retryable_build_error(rejection)
    (_, chosen), _, _ = _driver({"requested": rejection}, {"fits": 51184}, ["fits"])
    assert chosen == "fits"
